...
namespace leaf::pattern {
  ...
  template <typename... T> auto IObservable<T...>::subscribe(IObserver<T...>* observer) -> void
  {
    this->m_observers.push_back(observer);
    llog::trace("observable: subscriber added, {} total", this->m_observers.size());
  }

  template <typename... T> auto IObservable<T...>::unsubscribe(IObserver<T...>* observer) -> void
  {
    this->m_observers.remove(observer);
    llog::trace("observable: subscriber removed, {} total", this->m_observers.size());
  }

  template <typename... T> auto IObservable<T...>::notify(T... args) -> void
  {
  ...
  template <typename... T> auto IObservable<T...>::operator+=(IObserver<T...>* observer) -> void
  {
  ...
  template <typename... T> auto IObservable<T...>::operator~() -> void { this->notify(); }
} // namespace leaf::pattern
