...
namespace leaf::reflect
{
  namespace detail
  {
    ...
    template<auto...  Vs> [[nodiscard]] constexpr auto function_name() noexcept -> std::string_view { return lf::source_location::current().function_name(); }
    template<class... Ts> [[nodiscard]] constexpr auto function_name() noexcept -> std::string_view { return lf::source_location::current().function_name(); }
...
static_assert(([]<auto expect = [](const bool cond) { return std::array{true}[not cond]; }> {
  using namespace leaf::reflect;

  // nth_pack_element
  {
    using leaf::reflect::detail::nth_pack_element;
...
#pragma pack(pop)
  ...
  // for_each
  {
    ...
    auto i = 0;
    for_each([&](auto I) {
      size_of[i] = leaf::reflect::size_of<I>(f);
      align_of[i] = leaf::reflect::align_of<I>(f);
      offset_of[i] = leaf::reflect::offset_of<I>(f);
      name[i] = leaf::reflect::member_name<I>(f);
      type[i] = leaf::reflect::type_name(leaf::reflect::get<I>(f));
      value[i] = leaf::reflect::get<I>(f);
      ++i;
    }, f);
