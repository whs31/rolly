...
namespace rolly {
  ...
  namespace trait {
    ...
    namespace detail {
      ...
      struct is_callable<
        F,
        typelist<T...>,
        void_t<decltype(std::declval<F>()(std::declval<T>()...))>> : std::true_type {};
  ...
  namespace detail {
    ...
    struct func_ptr {
      ...
      void store(T const& t) {
        auto const* b = reinterpret_cast<char const*>(&t);
        sz = sizeof(T);
    ...
    struct function_traits {
      static void ptr(T const& /*t*/, func_ptr& /*d*/) {}

      static bool eq(T const& /*t*/, func_ptr const& /*d*/) { return false; }
    ...
    struct function_traits<T, std::enable_if_t<trait::is_func_v<T>>> {
      ...
      static bool eq(T& t, func_ptr const& d) {
        auto const* r = d.as<T const*>();
        return r && *r == &t;
      }
    ...
    struct function_traits<T*, std::enable_if_t<trait::is_func_v<T>>> {
      ...
      static bool eq(T* t, func_ptr const& d) { return function_traits<T>::eq(*t, d); }
    ...
    struct function_traits<T, std::enable_if_t<trait::has_call_operator_v<T>>> {
      ...
      static bool eq(T const& /*t*/, func_ptr const& d) {
    ...
    template <typename T>
    func_ptr get_function_ptr(T const& t) {
      func_ptr d;
    ...
    template <typename T>
    bool eq_function_ptr(T const& t, func_ptr const& d) {
    ...
    struct object_pointer {
      static obj_ptr get(T const&) { return nullptr; }
    };

    template <typename T>
    struct object_pointer<T*, std::enable_if_t<trait::is_pointer_v<T*>>> {
      static obj_ptr get(T const* t) { return reinterpret_cast<obj_ptr>(t); }
    };
    ...
    struct object_pointer<
      T,
      std::enable_if_t<
        ! trait::is_pointer_v<T> && ! trait::is_weak_ptr_v<T>
        && trait::is_weak_ptr_compatible_v<T>>> {
      static obj_ptr get(T const& t) { return t ? reinterpret_cast<obj_ptr>(t.get()) : nullptr; }
    };
     ...
     */
    template <typename T>
    T const& cow_read(T const& v) {
      return v;
    }

    template <typename T>
    T const& cow_read(copy_on_write<T>& v) {
      return v.read();
    }
    ...
    class slot_state {
     ...
     private:
      template <typename, typename...>
      friend class ::rolly::signal_base;

      std::size_t m_index;     // index into the array of slot pointers inside the signal
      group_id const m_group;  // slot group this slot belongs to
  ...
  class connection {
   public:
    ...
    connection_blocker blocker() const noexcept { return connection_blocker {m_state}; }
  ...
  class scoped_connection final : public connection {
   public:
    ...
    /*implicit*/ scoped_connection(connection const& c) noexcept
      : connection(c) {}
  ...
  class signal_base final : public detail::cleanable {
    ...
    template <typename U, typename L>
    using cow_copy_type =
      std::conditional_t<is_thread_safe<L>::value, detail::copy_on_write<U>, U const&>;
   ...
   public:
    ...
    std::
      enable_if_t<trait::is_callable_v<arg_list, Pmf, Ptr> && trait::is_observer_v<Ptr>, connection>
    ...
    template <typename Pmf, typename Ptr>
    std::enable_if_t<
      trait::is_callable_v<arg_list, Pmf, Ptr> && ! trait::is_observer_v<Ptr>
    ...
    template <typename Pmf, typename Ptr>
    std::enable_if_t<
      trait::is_callable_v<ext_arg_list, Pmf, Ptr> && ! trait::is_weak_ptr_compatible_v<Ptr>,
      connection>
    ...
    template <typename Pmf, typename Ptr>
    std::enable_if_t<
      ! trait::is_callable_v<arg_list, Pmf> && trait::is_weak_ptr_compatible_v<Ptr>,
      connection>
      connect(Pmf&& pmf, Ptr&& ptr, group_id gid = 0) {
      using trait::to_weak;
    ...
    std::enable_if_t<
      trait::is_callable_v<arg_list, Callable> && trait::is_weak_ptr_compatible_v<Trackable>,
      connection>
    ...
    std::enable_if_t<
      ...
      return disconnect_if([&](auto const& s) { return s->has_full_callable(c); });
    }
    ...
    std::enable_if_t<
      ! trait::is_callable_v<arg_list, Obj> && ! trait::is_callable_v<ext_arg_list, Obj>
        && ! trait::is_pmf_v<Obj>,
      size_t>
    ...
    size_t disconnect(Callable const& c, Obj const& obj) {
      return disconnect_if([&](auto const& s) { return s->has_object(obj) && s->has_callable(c); });
    }
  ...
  template <
    ...
    typename... Args>
  connection connect(
    signal_base<Lockable1, T1...>& sig1,
    signal_base<Lockable2, T2...>& sig2,
    Args&&... args
  ) {
    return sig1.connect(
      detail::signal_wrapper<signal_base<Lockable2, T2...>> {std::addressof(sig2)},
