  ...
  namespace io {
    class filedevice {
     public:
      ...
      [[nodiscard]] result<std::string> try_read() const noexcept { return filedevice::try_read_from(this->path()); }

      void write(std::string_view content) const noexcept(false) { filedevice::write_to(this->path(), content); }
      ...
      [[nodiscard]] static std::string read_from(std::filesystem::path const& path) {
        ...
        if(not handle.is_open())
          throw std::runtime_error("failed to open file handle for reading at " + path.generic_string());
        return {(std::istreambuf_iterator<char>(handle)), std::istreambuf_iterator<char>()};
      }

      [[nodiscard]] static result<std::string> try_read_from(std::filesystem::path const& path) noexcept {
        auto handle = std::ifstream(path);
        if(not handle.is_open())
          return error("failed to open file handle for reading at \'{}\'", path.generic_string());
        return ok<std::string>({(std::istreambuf_iterator<char>(handle)), std::istreambuf_iterator<char>()});
      }

      static void write_to(std::filesystem::path const& path, std::string_view content) noexcept(false) {
        ...
        if(not handle.is_open())
          throw std::runtime_error("failed to open file handle for writing at " + path.generic_string());
        if(fs::exists(path))
      ...
      [[nodiscard]] static result<> try_write_to(std::filesystem::path const& path, std::string_view content) noexcept {
        namespace fs = std::filesystem;
